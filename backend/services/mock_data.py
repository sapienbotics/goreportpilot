...
def generate_mock_ga4_data(client_name: str, period_start: str, period_end: str) -> Dict[str, Any]:
    """Generate realistic Google Analytics 4 data."""
    ...
    traffic_sources = [
        {"source": "Organic Search", "sessions": organic,  "percentage": round(organic  / source_total * 100, 1)},
        {"source": "Paid Search",    "sessions": paid,     "percentage": round(paid     / source_total * 100, 1)},
        {"source": "Social",         "sessions": social,   "percentage": round(social   / source_total * 100, 1)},
        {"source": "Direct",         "sessions": direct,   "percentage": round(direct   / source_total * 100, 1)},
        {"source": "Referral",       "sessions": referral, "percentage": round(referral / source_total * 100, 1)},
    ]

    # Top pages
    top_pages = [
        {"page": "/",               "sessions": int(sessions * 0.25), "bounce_rate": round(random.uniform(30, 50), 1)},
        {"page": "/products",       "sessions": int(sessions * 0.15), "bounce_rate": round(random.uniform(35, 55), 1)},
        {"page": "/blog/top-tips",  "sessions": int(sessions * 0.08), "bounce_rate": round(random.uniform(40, 65), 1)},
        {"page": "/contact",        "sessions": int(sessions * 0.06), "bounce_rate": round(random.uniform(25, 45), 1)},
        {"page": "/pricing",        "sessions": int(sessions * 0.05), "bounce_rate": round(random.uniform(30, 50), 1)},
    ]
...
def generate_mock_meta_ads_data(client_name: str, period_start: str, period_end: str) -> Dict[str, Any]:
    ...
    for i, name in enumerate(campaign_names):
        ...
        campaigns.append({
            ...
            "roas": round((c_conv * random.uniform(40, 150)) / max(c_spend, 1), 2),
        })
